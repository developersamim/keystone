...
using identity_server.Controllers.Account;
...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
            ...
            if (button != "login")
            {
                if (context != null)
                {
                    ...
                    if (context.IsNativeClient())
                    {
                        ...
                        return this.LoadingPage("Redirect", model.ReturnUrl, context.Client.ClientName);
                    }
        ...
        {
            var vm = BuildUserRegistrationViewModelAsync(returnUrl);
            return View(vm);
        }
        ...
        public async Task<IActionResult> Register(UserRegistrationModel userModel, string button)
        {
            ...
            // check if we are in the context of an authorization request
            var context = await _interaction.GetAuthorizationContextAsync(userModel.ReturnUrl);
            ...
            if (result.Succeeded)
            {
                ...
                if (context != null)
                {
                    if (context.IsNativeClient())
                    {
                        // The client is native, so this change in how to
                        // return the response is for better UX for the end user.
                        return this.LoadingPage("Redirect", userModel.ReturnUrl, context.Client.ClientName);
                    }
        ...
        {
            var model = new ResetPasswordModel { Token = token, Email = email, ReturnUrl = returnUrl};
            return View(model);
        }
        ...
        private UserRegistrationModel BuildUserRegistrationViewModelAsync(string returnUrl)
        {
